...
After wiring, verify the whole canvas via the API in one read: `GET /api/3/automations/{id}/blocks` returns the full structure under the `automationBlocks` key (NOT `blocks` - parse the right key). Check per block - `send` blocks carry `sendtype` (`"imm"` Immediately / `"opt"` Predictive) and `campaignid`; `wait` blocks carry `waittime` and `waitdurt`; `goal` blocks carry `name`, `position` ("anywhere") and `unmet` ("end"); `addtag`/`removetag` carry `tagIds`. Then GET every copy's campaign and message to confirm GA name and sender stuck. Full schema and the audited capability surface in `docs/api-capability-map.md` at the repo root.
